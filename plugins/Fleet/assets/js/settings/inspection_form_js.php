...
var admin_url = $('input[name="site_url"]').val();
 ...
 function training_toggle_full_view() {
  ...
  function add_inspection_question(type, training_id) {
      'use strict';
      var data = {};
      data.type = type;
      data.training_id = training_id;
      $.post(admin_url + 'fleet/add_inspection_question_form', data).done(function(response) {
        ...
        question_area += '<input type="checkbox" data-question_required="' + response.data.questionid + '" name="required[]" onchange="update_question(this,\'' + type + '\',' + response.data.questionid + ')" class="form-check-input">';
        ...
        question_area += '<label for="' + response.data.questionid + '" class="control-label display-block col-md-12">' + response.survey_question_string + ' <a href="#" onclick="update_question(this,\'' + type + '\',' + response.data.questionid + '); return false;" class="pull-right update-question-button ps-2"><i data-feather="refresh-cw" class="icon-18 text-success question_update"></i></a><a href="#" class="pull-right text-danger" onclick="remove_question_from_database(this,' + response.data.questionid + '); return false;"><i data-feather="x-circle" class="icon-18"></i></a></label>';
        ...
        if (type == 'textarea') {
            ...
            question_area += '<div class="' + type + ' ' + type + '-primary display-inline-block">';
            question_area += '<input type="' + type + '" disabled="disabled" onchange="update_answer_question(this,\'' + type + '\',' + response.data.questionid + ','+response.data[0].questionboxdescriptionid+');" data-checked-descriptionid="'+response.data[0].questionboxdescriptionid+'" class="data_checked_descriptionid form-check-input m-2"/>';
            question_area += '<label><input onblur="update_question(this,\'' + type + '\',' + response.data.questionid + ');" type="text" data-box-descriptionid="' + response.data[0].questionboxdescriptionid + '" class="survey_input_box_description form-control"></label>';
            question_area += '</div>';
            question_area += '<a href="#" class="survey_add_more_box" onclick="' + box_description_function + '"><i class="icon-18 m-2" data-feather="plus-circle"></i></a>';
            question_area += '<a href="#" class="add_remove_action hide text-danger" onclick="remove_box_description_from_database(this,' + response.data[0].questionboxdescriptionid + '); return false;"><i class="icon-18 m-2" data-feather="x-circle"></i></i></a>';
            question_area += '</div>';
            question_area += '</div>';
            // end box row
            question_area += '</div>';
            ...
            question_area += '<label><?php echo _l('pass_label'); ?>&nbsp;&nbsp;&nbsp;</label>';
            question_area += '<label><input onblur="update_question(this,\'' + type + '\',' + response.data.questionid + ');" type="text" data-box-descriptionid="' + response.data[0].questionboxdescriptionid + '" class="survey_input_box_description form-control" value="<?php echo _l('pass'); ?>"></label>';
            question_area += '</div>';
            question_area += '</div>';
            question_area += '<div class="box_area">';
            question_area += '<div class="col-md-12 mtop10">';
            question_area += '<div class="' + type + ' ' + type + '-primary">';
            question_area += '<label><?php echo _l('fail_label'); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>';
            question_area += '<label><input onblur="update_question(this,\'' + type + '\',' + response.data.questionid + ');" type="text" data-box-descriptionid="' + response.data[0].questionboxdescriptionid2 + '" class="survey_input_box_description form-control" value="<?php echo _l('fail'); ?>"></label>';
            question_area += '</div>';
            question_area += '</div>';
            question_area += '</div>';
        } else {
...
if (type == 'checkbox' || type == 'radio' || type == 'pass_fail') {
    var tempData = [];
...
setTimeout(function() {
    $.post(admin_url + 'fleet/update_inspection_question_form', data).done(function(response) {
...
function update_answer_question(question, type, questionid, answer_id) {
  ...
  setTimeout(function() {
    $.post(admin_url + 'fleet/update_training_question_answer', data).done(function(response) {
        $(question).parents('li').find('i.question_update').removeClass('spinning');
    });
}, 10);
}
...
function add_more_boxes(question, boxdescriptionid) {
  ...
  $(box).find('.survey_add_more_box').addClass('hide');
  $(box).find('.add_remove_action').attr('onclick', 'remove_box_description_from_database(this,' + boxdescriptionid + '); return false;').removeClass('hide');
...
function remove_question_from_database(question, questionid) {
  'use strict';
  $.get(admin_url + 'fleet/remove_question/' + questionid, function(response) {
...
function remove_box_description_from_database(question, questionboxdescriptionid) {
  'use strict';
  $.get(admin_url + 'fleet/remove_box_description/' + questionboxdescriptionid, function(response) {
...
function add_box_description_to_database(question, questionid, boxid) {
  'use strict';
  $.get(admin_url + 'fleet/add_box_description/' + questionid + '/' + boxid, function(response) {
...
function update_questions_order() {
  ...
  var data = {};
  data.data = update;
  $.post(admin_url + 'fleet/update_inspection_questions_orders', data);
}
