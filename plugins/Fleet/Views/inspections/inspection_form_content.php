  ...
  <div class="col-md-12">
     <?php if(count($inspection_form->questions) > 0){
          ...
          if(isset($inspection_results)){
            foreach($inspection_results as $result){
              if($result['boxdescriptionid'] != null){
                  $value_result[$result['questionid']][] = $result['boxdescriptionid'];
               }else{
                  $value_result[$result['questionid']] = $result['answer'];
               }
            }
          }
        foreach($inspection_form->questions as $question){
         ...
         $question_area .= '<textarea class="form-control" rows="6" name="question['.$question['questionid'].'][]" data-for="'.$question['questionid'].'" id="'.$question['questionid'].'" data-required="'.$question['required'].'" value="'.(is_string($value) ? $value : '').'">'.(is_string($value) ? $value : '').'</textarea>';
          ...
          foreach($question['box_descriptions'] as $box_description){
           ...
           name="selectable['.$question['boxid'].']['.$question['questionid'].'][]" value="'.$box_description['questionboxdescriptionid'].'" data-required="'.$question['required'].'" id="chk_'.$question['boxtype'].'_'.$box_description['questionboxdescriptionid'].'" '.$checked.' class="form-check-input"/>';
           $question_area .= '
         ...
         $question_area .= '</div>';
          ...
          foreach($question['box_descriptions'] as $box_description){
           ...
           name="selectable['.$question['boxid'].']['.$question['questionid'].'][]" value="'.$box_description['questionboxdescriptionid'].'" data-required="'.$question['required'].'" id="chk_radio_'.$box_description['questionboxdescriptionid'].'" '.$checked.' class="form-check-input"/>';
           $question_area .= '
